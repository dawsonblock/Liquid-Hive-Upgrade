  ...
  return (
    <Grid container spacing={3}>
            ...
            {history.length === 0 && (
              <Box sx={{ textAlign: 'center', color: 'text.secondary', mt: 8 }}>
                <Typography variant="h5" gutterBottom sx={{ fontWeight: 700 }}>
                  Welcome to <Box component="span" sx={{ color: 'primary.main' }}>LIQUID‑HIVE</Box>
                </Typography>
                <Typography variant="body2">Ask anything. Press Enter to send, Shift+Enter for a new line.</Typography>
      ...
      {history.map((m, i) => {
              ...
              return (
        <Stack key={i} direction="row" spacing={1.5} sx={{ mb: 2, alignItems: 'flex-start', opacity: 0, animation: 'fadeIn 200ms ease forwards' }}>
                  <Avatar sx={{ width: 28, height: 28, mt: 0.25, bgcolor: isUser ? 'primary.main' : 'secondary.main', color: 'primary.contrastText' }}>
                    {isUser ? 'U' : 'A'}
                  </Avatar>
                  <Box
                    sx={{
                    ...
                    {!isUser && (
                      <Stack direction="row" spacing={1} sx={{ mt: 0.5 }}>
                        <Tooltip title={copiedIdx === i ? 'Copied' : 'Copy message'}>
                          <span>
                            <IconButton size="small" onClick={() => handleCopy(m.content, i)} aria-label="Copy message">
                        ...
                        <Tooltip title="Regenerate response">
                          <span>
                            <IconButton size="small" onClick={() => handleRegenerate(i)} aria-label="Regenerate response">
                      ...
                      <Accordion sx={{ mt: 1 }}>
                        ...
                        <AccordionDetails>
                          {lastCritique && (<>
                            ...
                            <Typography variant="body2" color="text.secondary">{lastCritique}</Typography>
                          </>)}
                          {lastCorrection && (<>
                            <Typography variant="subtitle2" sx={{ mt: 1 }}>Correction Analysis</Typography>
                            <Typography variant="body2" color="text.secondary">{lastCorrection}</Typography>
                          </>)}
                        </AccordionDetails>
                      </Accordion>
                    )}
          ...
          <Paper variant="outlined" sx={{ p: 1, borderRadius: 999, px: 1.5 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <IconButton color="primary" onClick={sendText} aria-label="Send message"><SendIcon /></IconButton>
              <Button variant="outlined" onClick={sendVision} disabled={!imageFile}>Send Image</Button>
            </Stack>
            <FormControlLabel sx={{ ml: 0.5, mt: 0.5 }} control={<Switch checked={groundingRequired} onChange={e => setGroundingRequired(e.target.checked)} />} label="Grounding Required" />
          </Paper>
        </Stack>
      </Grid>
