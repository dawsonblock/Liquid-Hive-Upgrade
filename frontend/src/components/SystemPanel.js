...
const SystemPanel = () => {
  ...
  const handlePreview = async () => {
    try {
      ...
      if (!res?.candidates?.length)
        setSnack(prev => [
          ...prev,
          { open: true, msg: 'No auto‑promotion candidates at this time.' },
        ]);
    } catch (e) {
  ...
  const onApplyThresholds = async () => {
    ...
    try {
      ...
      setSnack(prev => [
        ...prev,
        { open: true, msg: r.error ? `Failed: ${r.error}` : 'Thresholds updated' },
      ]);
    } catch {
        ...
        children: [
              ...
              children: [
                _jsxs(Stack, {
                  ...
                  children: [
                    _jsx(Typography, {
                      variant: 'h6',
                      children: 'Core Vitals & Self\u2011Awareness (\u03A6)',
                    }),
                    _jsx(Button, {
                      size: 'small',
                      variant: 'outlined',
                      onClick: handlePreview,
                      children: 'Preview Auto\u2011Promotions',
                    }),
                  ],
                }),
                _jsxs(Stack, {
                  ...
                  sx: { mt: 1 },
                  children: [
                    _jsx(Chip, { label: `Phi: ${vitals.phi ?? 'N/A'}`, color: 'primary' }),
                    _jsx(Chip, { label: `Memories: ${stateSummary?.memory_size ?? 'N/A'}` }),
                  ],
                }),
                _jsxs(Box, {
                  sx: { mt: 2 },
                  children: [
                    _jsx(Typography, { variant: 'subtitle2', children: 'Operator Intent' }),
                    _jsx(Typography, {
                      variant: 'body2',
                      color: 'text.secondary',
                      children: stateSummary?.operator_intent ?? 'Unknown',
                    }),
                  ],
                }),
                preview.length > 0 &&
                  _jsxs(Box, {
                    ...
                    children: [
                      _jsx(Typography, {
                        variant: 'subtitle2',
                        children: 'Auto\u2011Promotion Candidates',
                      }),
                      _jsx(List, {
                        dense: true,
                        children: preview.map((p, i) =>
                          _jsx(
                            ListItem,
                            {
                              children: _jsx(ListItemText, {
                                primary: `Role ${p.role}: ${p.challenger} vs ${p.active}`,
                                secondary: `p95 ${p.p95_c?.toFixed?.(3)}s vs ${p.p95_a?.toFixed?.(3)}s; cost ${p.cost_c?.toFixed?.(6)} vs ${p.cost_a?.toFixed?.(6)}`,
                              }),
                            },
                            i
                          )
          ...
          _jsx(Grid, {
            ...
            children: _jsxs(Paper, {
              ...
              sx: { p: 2 },
              children: [
                _jsx(Typography, { variant: 'h6', children: 'Approval Queue' }),
                _jsxs(List, {
                  children: [
                    approvals.map(a =>
                      _jsx(
                        ListItem,
                        {
                          secondaryAction: _jsxs(Box, {
                            children: [
                              _jsx(IconButton, {
                                color: 'success',
                                onClick: async () => {
                                  await approveProposal(a.id);
                                  setApprovals(await getApprovals());
                                },
                          ...
                          children: _jsx(ListItemText, { primary: a.content }),
                        },
                        a.id
                      )
                    ),
        ...
        children: [
            ...
            children: _jsxs(Paper, {
              ...
              children: [
                _jsxs(Stack, {
                  ...
                  alignItems: 'center',
                  children: [
                    _jsx(Typography, { variant: 'h6', children: 'Swarm Status' }),
                    _jsx(Button, {
                      size: 'small',
                      variant: 'outlined',
                      onClick: async () => {
                ...
                _jsx(Typography, {
                  ...
                  sx: { mt: 1 },
                  children: swarm
                    ? `Enabled: ${String(swarm.swarm_enabled)} • Nodes: ${swarm.active_nodes ?? 0}`
                    : 'No swarm info',
                }),
                swarm?.nodes &&
                  _jsx(Typography, {
                    ...
                    children: JSON.stringify(swarm.nodes, null, 2),
                  }),
              ],
            }),
          }),
          _jsx(Grid, {
            ...
            children: _jsxs(Paper, {
              ...
              sx: { p: 2 },
              children: [
                _jsx(Typography, { variant: 'h6', children: 'Router Thresholds' }),
                _jsxs(Stack, {
                  ...
                  children: [
                    ...
                    _jsx(TextField, {
                      ...
                      label: 'Max CoT',
                      value: maxCot,
                      onChange: e => setMaxCot(e.target.value),
                      sx: { width: 120 },
                    }),
                  ],
                }),
                ...
                _jsx(Typography, {
                  variant: 'caption',
                  color: 'text.secondary',
                  children: 'Requires ADMIN_TOKEN configured on server.',
                }),
              ],
            }),
          }),
      ...
      snack.map((s, idx) =>
        _jsx(
          Snackbar,
          {
            ...
            onClose: () => setSnack(prev => prev.filter((_, i) => i !== idx)),
            children: _jsx(Alert, {
              onClose: () => setSnack(prev => prev.filter((_, i) => i !== idx)),
              severity: 'info',
              sx: { width: '100%' },
