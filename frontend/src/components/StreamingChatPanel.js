...
import { Accordion, AccordionDetails, AccordionSummary, Alert, Box, Button, Chip, CircularProgress, FormControlLabel, Grid, IconButton, Paper, Stack, Switch, TextField, Typography, Tooltip } from '@mui/material';
...
const StreamingChatPanel = () => {
    ...
    const resendForRegeneration = useCallback((idx) => {
    ...
    return (_jsxs(Grid, { container: true, spacing: 3, children: [_jsx(Grid, { item: true, xs: 12, md: 8, children: _jsxs(Stack, { spacing: 2, children: [_jsx(Paper, { variant: "outlined", sx: { p: 1.5, bgcolor: 'background.paper', borderRadius: 4 }, children: _jsxs(Stack, { direction: "row", spacing: 2, alignItems: "center", children: [_jsx(Chip, { icon: connectionStatus === 'connecting' ? _jsx(CircularProgress, { size: 16 }) : _jsx(StreamIcon, {}), label: `Streaming: ${connectionStatus}`, color: getConnectionStatusColor(), variant: "outlined", size: "small" }), streamMetadata && (_jsx(Chip, { icon: _jsx(CacheIcon, {}), label: streamMetadata.cached ? (`Cache Hit${typeof streamMetadata.cache_similarity === 'number' ? ` (${(streamMetadata.cache_similarity * 100).toFixed(0)}%)` : ''}`) : 'Live Generation', color: streamMetadata.cached ? 'success' : 'primary', variant: "outlined", size: "small" })), cacheAnalytics && (_jsx(Chip, { icon: _jsx(CacheIcon, {}), label: `Cache: ${Math.round((cacheAnalytics.hit_rate || 0) * 100)}%`, color: (cacheAnalytics.hit_rate || 0) > 0.5 ? 'success' : 'default', variant: "outlined", size: "small" })), isStreaming && (_jsx(Typography, { variant: "caption", color: "primary", children: "Generating response..." })), _jsx(Box, { sx: { flex: 1 } }), isStreaming && (_jsx(Button, { size: "small", color: "error", variant: "outlined", onClick: stopGeneration, startIcon: _jsx(StopIcon, {}), children: "Stop" })), _jsx(Button, { size: "small", variant: "outlined", onClick: refreshProviders, startIcon: providersLoading ? _jsx(CircularProgress, { size: 14 }) : _jsx(RefreshIcon, {}), children: "Refresh Providers" }), _jsx(Button, { size: "small", variant: "outlined", onClick: refreshCacheAnalytics, startIcon: cacheLoading ? _jsx(CircularProgress, { size: 14 }) : _jsx(CacheIcon, {}), children: "Cache Analytics" })] }) }), providers && Object.keys(providers).length > 0 && (_jsx(Paper, { variant: "outlined", sx: { p: 1 }, children: _jsx(Stack, { direction: "row", spacing: 1, alignItems: "center", flexWrap: "wrap", children: Object.entries(providers).map(([name, info]) => (_jsx(Chip, { size: "small", label: `${name}: ${info?.status || 'unknown'}`, color: (info?.status === 'healthy' ? 'success' : (info?.status ? 'warning' : 'default')) }, name))) }) })), _jsxs(Paper, { variant: "outlined", sx: { p: 2.5, height: 520, overflowY: 'auto', bgcolor: (t) => t.palette.mode === 'dark' ? 'rgba(2,6,23,0.6)' : 'rgba(255,255,255,0.9)', borderRadius: 4 }, children: [history.length === 0 && (_jsx(Typography, { variant: "body2", color: "text.secondary", children: "Start the conversation with the Enhanced Cognitive Core. Streaming is enabled for real-time responses." })), history.map((message, i) => (_jsxs(Box, { sx: { mb: 2 }, children: [_jsxs(Stack, { direction: "row", spacing: 1, alignItems: "center", children: [_jsx(Typography, { variant: "caption", color: "text.secondary", children: message.role.toUpperCase() }), message.cached && (_jsx(Chip, { label: "CACHED", color: "success", size: "small" })), message.provider && (_jsx(Chip, { label: message.provider, color: "info", size: "small" })), message.isStreaming && (_jsx(CircularProgress, { size: 12 }))] }), _jsx(Box, { sx: { mt: 1, px: 1.25, py: 1, borderRadius: 3, bgcolor: (t) => message.role === 'user' ? (t.palette.mode === 'dark' ? 'rgba(26,115,232,0.14)' : 'rgba(26,115,232,0.08)') : t.palette.background.paper, border: (t) => `1px solid ${t.palette.divider}`, opacity: 0, animation: 'fadeIn 200ms ease forwards' }, children: _jsx(Markdown, { children: message.role === 'assistant' && message.isStreaming
                                                    ? (currentStreamingMessage || message.content)
                                                    : message.content }) }), message.role === 'assistant' && (_jsxs(Stack, { direction: "row", spacing: 1, sx: { mt: 0.5 }, children: [_jsx(Tooltip, { title: copiedIdx === i ? 'Copied' : 'Copy message', children: _jsx("span", { children: _jsx(IconButton, { size: "small", onClick: () => copyToClipboard(message.content, i), "aria-label": "Copy message", children: copiedIdx === i ? _jsx(CheckIcon, { fontSize: "small" }) : _jsx(ContentCopyIcon, { fontSize: "small" }) }) }) }), _jsx(Tooltip, { title: "Regenerate response", children: _jsx("span", { children: _jsx(IconButton, { size: "small", onClick: () => resendForRegeneration(i), "aria-label": "Regenerate response", disabled: connectionStatus !== 'connected', children: _jsx(ReplayIcon, { fontSize: "small" }) }) }) })] })), message.role === 'assistant' && message.isStreaming && isStreaming && (_jsx(Box, { sx: { mt: 1 }, children: _jsx(Typography, { variant: "caption", color: "primary", children: "\u258B " }) })), message.metadata && (_jsxs(Accordion, { sx: { mt: 1 }, children: [_jsx(AccordionSummary, { expandIcon: _jsx(ExpandMoreIcon, {}), children: _jsx(Typography, { variant: "caption", children: "Show Details" }) }), _jsx(AccordionDetails, { children: _jsx("pre", { style: { fontSize: '0.75rem', maxHeight: '200px', overflow: 'auto' }, children: JSON.stringify(message.metadata, null, 2) }) })] }))] }, i))), _jsx("div", { ref: messagesEndRef })] }), _jsxs(Stack, { direction: "row", spacing: 1, alignItems: "center", children: [_jsx(TextField, { fullWidth: true, size: "small", placeholder: isStreaming ? "Generating response..." : "Type a message...", value: input, onChange: e => setInput(e.target.value), disabled: isStreaming, onKeyDown: (e) => {
