...
const ForgePanel: React.FC = () => {
  ...
  const [gov, setGov] = useState<{
    ENABLE_ORACLE_REFINEMENT?: boolean;
    FORCE_DEEPSEEK_R1_ARBITER?: boolean;
  }>({});
  ...
  const onToggleGov = async (
    key: 'ENABLE_ORACLE_REFINEMENT' | 'FORCE_DEEPSEEK_R1_ARBITER',
    value: boolean
  ) => {
        ...
        <Paper variant='outlined' sx={{ p: 2 }}>
          <Stack direction='row' spacing={2} alignItems='center'>
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={!!gov.FORCE_DEEPSEEK_R1_ARBITER}
                  onChange={e => onToggleGov('FORCE_DEEPSEEK_R1_ARBITER', e.target.checked)}
                />
              }
              label='Force DeepSeek‑R1 Arbiter'
            />
            <Box flexGrow={1} />
