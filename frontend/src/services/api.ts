...
export const warmQwenProvider = (adminToken?: string) =>
  api.post('/admin/providers/qwen/warm', {}, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined
...
export const optimizeCache = (targetHitRate: number) =>
  api.post('/cache/optimize', null, { params: { target_hit_rate: targetHitRate } }).then(r => r.data as any);
...
export const setRouterThresholds = (
  ...
  adminToken?: string
) => api.post('/admin/router/set-thresholds', params, {
  headers: adminToken ? { 'x-admin-token': adminToken } : undefined
