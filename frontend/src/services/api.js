...
export const warmQwenProvider = (adminToken) => api.post('/admin/providers/qwen/warm', {}, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined
}).then(r => r.data);
// Providers status
...
export const clearCache = (pattern, adminToken) => api.post('/cache/clear', { pattern }, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined
}).then(r => r.data);
...
export const optimizeCache = (targetHitRate) => api.post('/cache/optimize', null, { params: { target_hit_rate: targetHitRate } }).then(r => r.data);
export const warmCache = () => api.post('/cache/warm').then(r => r.data);
// Router thresholds (admin)
export const setRouterThresholds = (params, adminToken) => api.post('/admin/router/set-thresholds', params, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined
}).then(r => r.data);
// Swarm status
