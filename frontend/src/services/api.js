...
export const postVision = (form, groundingRequired) => api
...
export const secretExists = (name) => api
    .get(`/secrets/exists`, { params: { name } })
    .then(r => r.data);
export const setSecret = (name, value, adminToken) => api
    .post('/secrets/set', { name, value }, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined,
})
    .then(r => r.data);
export const reloadRouterSecrets = (adminToken) => api
    .post('/admin/router/reload-secrets', {}, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined,
})
    .then(r => r.data);
// Warm Qwen CPU provider (loads local model)
export const warmQwenProvider = (adminToken) => api
    .post('/admin/providers/qwen/warm', {}, {
    headers: adminToken ? { 'x-admin-token': adminToken } : undefined,
})
    .then(r => r.data);
// Providers status
...
export const clearCache = (pattern, adminToken) => api
...
export const optimizeCache = (targetHitRate) => api
...
export const setRouterThresholds = (params, adminToken) => api
