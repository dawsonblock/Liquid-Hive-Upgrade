...
class PreGuard:
    ...
    def _compile_risk_patterns(self) -> Dict[str, Pattern[str]]:
        ...
        return {
            ...
            "illegal": re.compile(r'\b(?:drug|illegal|steal|fraud|hack|exploit|piracy|copyright)\b', re.IGNORECASE),
            "self_harm": re.compile(r'\b(?:suicide|self-harm|cut myself|end my life|kill myself)\b', re.IGNORECASE),
            "hate_speech": re.compile(r'\b(?:racist|sexist|homophobic|transphobic|hate|discrimination)\b', re.IGNORECASE),
    ...
    async def process(self, request: GenRequest) -> Tuple[GenRequest, PreGuardResult]:
        ...
        if injection_detected:
            return request, PreGuardResult(
                blocked=True,
                reason=f"prompt_injection: {injection_reason}",
                status="blocked"
            )
        
        # Step 2: Detect and redact PII
        cleaned_prompt, pii_info = self._redact_pii(request.prompt)
        cleaned_system = self._redact_pii(request.system_prompt or "")[0] if request.system_prompt else request.system_prompt
        ...
        result = PreGuardResult(
            ...
            status="blocked" if should_block else ("sanitized" if pii_info["redacted"] else "passed"),
