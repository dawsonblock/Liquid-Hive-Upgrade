...
class ToolRegistry:
    ...
    def register_tool(self, tool: BaseTool) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to register tool {getattr(tool, 'name', 'unknown')}: {e}"
            )
            return False
    ...
    async def execute_tool(
        ...
        self._record_execution_metrics(
            name, result, execution_time, parameters, operator_id
        )

        # Log execution details
        self._log_tool_usage(
            name, parameters, result, execution_time, operator_id, execution_id
        )
    ...
    def approve_tool_execution(self, approval_id: str, approver_id: str) -> bool:
        ...
        for approval_key, approval in self.pending_approvals.items():
            if approval["approval_id"] == approval_id:
                ...
                self.logger.info(
                    f"Approved tool execution: {approval_id} by {approver_id}"
                )
                return True
    ...
    def deny_tool_execution(
        ...
        for approval_key, approval in self.pending_approvals.items():
            if approval["approval_id"] == approval_id:
                ...
                self.logger.info(
                    f"Denied tool execution: {approval_id} by {approver_id}"
                )
                return True
    ...
    def get_tool_analytics(
        self, tool_name: Optional[str] = None, days: int = 7
    ) -> Dict[str, Any]:
    ...
    def _get_most_used_tools(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return sorted(tool_usage, key=lambda x: x["total_executions"], reverse=True)[
            :limit
        ]
