...
class FileOperationsTool(BaseTool):
    ...
    async def _read_file(
        self, path: str, encoding: str, max_size_mb: int
    ) -> ToolResult:
        """Read file content."""
    ...
    async def _file_exists(self, path: str) -> ToolResult:
        ...
        return ToolResult(
            success=True, data=exists, metadata={"path": path, "operation": "exists"}
        )
    ...
    async def _delete_file(self, path: str) -> ToolResult:
        ...
        if file_path.is_dir():
            return ToolResult(
                success=False, error="Cannot delete directories with this tool"
            )
