...
class ConfidenceModeler:
    def __init__(self, policy: TrustPolicy) -> None:
        ...
        self.historical_actions: List[Dict[str, Any]] = (
            []
        )  # Full action history with outcomes
        self.cost_risk_history: Dict[str, List[float]] = (
            {}
        )  # action_type -> [cost/risk values]
    ...
    def _calculate_semantic_similarity(
        self, proposal_text: str, action_type: str
    ) -> float:
    ...
    def decide(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        bypass = (
            self.policy.enabled
            and (
                action_type in self.policy.allowlist if self.policy.allowlist else True
            )
