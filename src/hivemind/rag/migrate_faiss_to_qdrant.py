...
class FaissToQdrantMigrator:
    ...
    async def migrate(self, backup_faiss: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if not faiss_data["success"]:
                results["errors"].append(f"Failed to load FAISS data: {faiss_data['error']}")
                return results
            
            results["faiss_documents"] = len(faiss_data["documents"])
            
            # Convert FAISS documents to Qdrant format
            qdrant_documents = await self._convert_documents(faiss_data["documents"], faiss_data["vectors"])
            ...
            self.logger.info(f"Migration completed: {results['migrated_documents']} documents migrated")
    ...
    async def _backup_faiss_index(self):
        ...
        try:
            backup_dir = self.faiss_index_dir.parent / f"faiss_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    async def _load_faiss_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not index_path.exists() or not doc_store_path.exists():
                return {"success": False, "error": "FAISS index or document store not found"}
    ...
    async def _convert_documents(self, documents: List[Document], 
                                vectors: List[np.ndarray]) -> List[Dict[str, Any]]:
        ...
        for i, (doc, vector) in enumerate(zip(documents, vectors)):
            try:
                ...
                if "created_at" not in enhanced_metadata:
                    enhanced_metadata["created_at"] = enhanced_metadata["migration_timestamp"]
    ...
    async def _upload_to_qdrant(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if points:
                uploaded_count = await self.qdrant_retriever._batch_upload_points(points)
    ...
    async def verify_migration(self, sample_queries: List[str] = None) -> Dict[str, Any]:
        ...
        if verification_results["successful_searches"] > 0:
            verification_results["average_results_per_query"] = total_results / verification_results["successful_searches"]
        
        return verification_results

async def run_migration(faiss_index_dir: str = "/app/rag_index", 
                       qdrant_url: str = "http://localhost:6333",
                       embed_model: str = "all-MiniLM-L6-v2") -> Dict[str, Any]:
    ...
    if migration_results["migrated_documents"] > 0:
        ...
        print(f"   - Successful searches: {verification['successful_searches']}/{verification['total_queries']}")
