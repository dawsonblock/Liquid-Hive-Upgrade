...
class HybridRetriever:
    ...
    def _initialize_retrievers(
        self, faiss_index_dir: str, embed_model_id: str, qdrant_url: str
    ):
    ...
    def _determine_mode(self):
        ...
        if self.mode == RetrievalMode.AUTO:
            if qdrant_ready and faiss_ready:
                ...
                log.info(
                    "🔗 Operating in HYBRID mode (both FAISS and Qdrant available)"
                )
            elif qdrant_ready:
        ...
        self.is_ready = (
            self.mode in [RetrievalMode.FAISS_ONLY, RetrievalMode.HYBRID]
            and faiss_ready
        ) or (
            self.mode in [RetrievalMode.QDRANT_ONLY, RetrievalMode.HYBRID]
            and qdrant_ready
        )
        ...
        try:
            if self.mode == RetrievalMode.QDRANT_ONLY:
                ...
                return await self._search_hybrid(
                    query, k, metadata_filter, prefer_qdrant
                )
        ...
        if self.qdrant_retriever:
            ...
            for i, doc in enumerate(documents):
                ...
                context_lines.append(
                    f"[{i+1}] Source: {source} (Backend: {backend})\n{snippet}"
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.faiss_retriever:
            status["faiss"] = {
                ...
                "doc_count": (
                    len(self.faiss_retriever.doc_store)
        ...
        if self.faiss_retriever:
            try:
                ...
                health["backends"]["faiss"] = {
                    "status": (
                        "healthy" if self.faiss_retriever.is_ready else "unhealthy"
                    ),
                    "ready": self.faiss_retriever.is_ready,
