...
class HybridRetriever:
    ...
    def _determine_mode(self):
        ...
        self.is_ready = (
            (self.mode in [RetrievalMode.FAISS_ONLY, RetrievalMode.HYBRID] and faiss_ready) or
            (self.mode in [RetrievalMode.QDRANT_ONLY, RetrievalMode.HYBRID] and qdrant_ready)
        )
    ...
    async def search(self, query: str, k: int = 5, 
                    ...
                    prefer_qdrant: bool = True) -> List[Document]:
        """
    ...
    async def _search_qdrant_only(self, query: str, k: int, 
                                 metadata_filter: Optional[Dict[str, Any]]) -> List[Document]:
    ...
    async def _search_hybrid(self, query: str, k: int, 
                           metadata_filter: Optional[Dict[str, Any]],
                           prefer_qdrant: bool) -> List[Document]:
        ...
        if self.faiss_retriever and self.faiss_retriever.is_ready:
            faiss_task = asyncio.create_task(
                self.faiss_retriever.search(query, faiss_k)
            )
    ...
    def _fuse_results(self, qdrant_results: List[Document], 
                     faiss_results: List[Document], k: int) -> List[Document]:
    ...
    async def add_documents(self, file_paths: List[str], 
                          ...
                          use_qdrant: bool = True) -> Dict[str, Any]:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.faiss_retriever:
            status["faiss"] = {
                ...
                "doc_count": len(self.faiss_retriever.doc_store) if self.faiss_retriever.doc_store else 0
            }
