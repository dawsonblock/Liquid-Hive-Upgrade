                ...
                try:
                    ...
                    if file_results["success"]:
                        ...
                        results["errors"].append(
                            f"{file_path}: {file_results['error']}"
                        )
        ...
        for i in range(0, len(points), self.batch_size):
            ...
            try:
                ...
                if operation_info.status == UpdateStatus.COMPLETED:
                    ...
                    log.debug(
                        f"Uploaded batch {i//self.batch_size + 1}: {len(batch)} points"
                    )
                else:
                    log.warning(
    ...
    def _build_filter(self, metadata_filter: Dict[str, Any]) -> Filter:
        ...
        for key, value in metadata_filter.items():
            ...
            if isinstance(value, dict):
                ...
                if "gte" in value:
                    conditions.append(
                        FieldCondition(key=field_key, range=Range(gte=value["gte"]))
                    )
                if "lte" in value:
                    conditions.append(
                        FieldCondition(key=field_key, range=Range(lte=value["lte"]))
                    )
                if "gt" in value:
                    conditions.append(
                        FieldCondition(key=field_key, range=Range(gt=value["gt"]))
                    )
                if "lt" in value:
                    conditions.append(
                        FieldCondition(key=field_key, range=Range(lt=value["lt"]))
                    )
            else:
                # Handle exact match
                conditions.append(
