...
try:
    ...
    logging.getLogger(__name__).warning(
        f"Semantic cache dependencies not available: {e}"
    )
    ...
    def __init__(
        ...
        if semcache_deps_available:
            ...
            if self.enable_memory_fallback:
                self.is_ready = True
                log.info(
                    "⚠️ Semantic cache running in in-memory fallback mode (no Redis/NumPy)"
                )
    ...
    async def _initialize(self):
        ...
        try:
            ...
            try:
                ...
                self.embedding_model = None
                log.warning(
                    f"Embedding model load failed; will use lightweight fallback: {e}"
                )
    ...
    async def get(
        ...
        try:
            ...
            if similar_entry:
                ...
                response["cache_similarity"] = getattr(
                    similar_entry, "last_similarity", 1.0
                )
        ...
        try:
            ...
            try:
                if isinstance(query_embedding, list):
                    embedding_list = [
                        float(x) for x in cast(List[float], query_embedding)
                    ]
    ...
    def _get_embedding(self, text: str) -> Optional[Any]:
        ...
        if self.embedding_model is not None:
            try:
                ...
                log.warning(
                    f"Failed to generate embedding via model, using fallback: {e}"
                )
    ...
    async def _find_similar_entry(
        ...
        try:
            ...
            for entry in cached_entries:
                ...
                if (
                    similarity >= entry.similarity_threshold
            ...
            if best_entry:
                ...
                self.stats["average_similarity"] = (
                    self.stats["average_similarity"] * self.stats["cache_hits"]
                    + best_similarity
                ) / (self.stats["cache_hits"] + 1)
    ...
    async def _save_analytics(self):
        ...
        try:
            if self.redis_client is None:
                return
            await self.redis_client.setex(
                self.analytics_key, 86400, json.dumps(self.stats)  # 24 hours
            )
        except Exception as e:
    ...
    async def clear_cache(self, pattern: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if self.redis_client is None:
                if pattern:
                    to_del = [
                        ...
                        if pattern in k or pattern in v.query
                    ]
                    for k in to_del:
            ...
            if pattern:
                keys = cast(
                    List[str],
                    await self.redis_client.keys(
                        f"{self.metadata_key_prefix}:*{pattern}*"
                    ),
                )
            else:
                keys = cast(
                    List[str],
                    await self.redis_client.keys(f"{self.metadata_key_prefix}:*"),
                )
