...
class CacheManager:
    ...
    async def warm_cache(self, queries_and_responses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Cache warming completed: {results['cached_successfully']}/{results['total_queries']} cached in {results['warming_time']:.2f}s")
        ...
        try:
            ...
            analysis = {
                ...
                "resource_usage": {
                    ...
                    "cache_utilization": analytics.get("current_size", 0) / max(analytics.get("max_size", 1), 1)
                },
                "recommendations": []
            }
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if hit_rate < 0.1:
            recommendations.append("Very low cache hit rate - consider adjusting similarity threshold or caching strategy")
        elif hit_rate < 0.3:
            recommendations.append("Low cache hit rate - consider lowering similarity threshold or expanding cache warming")
        elif hit_rate > 0.8:
            recommendations.append("Excellent cache hit rate - current configuration is optimal")
        
        # Similarity threshold recommendations
        if similarity > 0.98:
            recommendations.append("Very high average similarity - consider slightly lowering threshold for better coverage")
        elif similarity < 0.85:
            recommendations.append("Low average similarity - consider raising threshold for better precision")
        
        # Cache utilization recommendations
        if utilization > 0.9:
            recommendations.append("Cache near capacity - consider increasing max cache size or reducing TTL")
        elif utilization < 0.3:
            recommendations.append("Low cache utilization - consider more aggressive caching strategy")
    ...
    async def optimize_cache_settings(self, target_hit_rate: float = 0.5) -> Dict[str, Any]:
        ...
        try:
            ...
            if results["changes_made"]:
                results["expected_improvement"] = min(0.2, max(0.05, target_hit_rate - current_hit_rate))
            
            self.logger.info(f"Cache optimization completed: {len(results['changes_made'])} changes made")
    ...
    async def import_cache_data(self, import_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Cache import completed: {results['imported_entries']} entries imported")
