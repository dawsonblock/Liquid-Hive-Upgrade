...
class CacheManager:
    ...
    async def optimize_cache_settings(
        self, target_hit_rate: float = 0.5
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if current_hit_rate < target_hit_rate and current_similarity > 0.95:
                ...
                results["changes_made"].append(
                    f"Lowered similarity threshold to {new_threshold}"
                )
                ...
                self.cache.similarity_threshold = new_threshold
                results["changes_made"].append(
                    f"Raised similarity threshold to {new_threshold}"
                )
            ...
            if current_hit_rate < target_hit_rate * 0.5:
                if self.cache.strategy != CacheStrategy.AGGRESSIVE:
                    ...
                    results["changes_made"].append(
                        "Switched to AGGRESSIVE caching strategy"
                    )
                ...
                if self.cache.strategy != CacheStrategy.SELECTIVE:
                    ...
                    results["changes_made"].append(
                        "Switched to SELECTIVE caching strategy"
                    )
