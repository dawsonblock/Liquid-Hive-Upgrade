...
def load_mixture(cfg: DataConfig) -> (Dataset, Optional[Dataset]):
    ...
    for ds_name in cfg.datasets:
        ...
        train_ds = (
            ds[split_name]
            if split_name in ds
            else (ds.get("train") or next(iter(ds.values())))
        )
            ...
            if eval_ds is not None:
                if cfg.max_eval_samples:
                    eval_ds = eval_ds.select(
                        range(min(len(eval_ds), cfg.max_eval_samples))
                    )
                eval_parts.append(eval_ds)

    def map_to_text(d: Dataset) -> Dataset:
        return d.map(
            lambda x: {"text": _format_sample(x)}, remove_columns=d.column_names
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        action="append",
        help="HF dataset repo id. Can be passed multiple times.",
    )
