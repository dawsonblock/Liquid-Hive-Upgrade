...
class AutonomyOrchestrator:
                ...
                trust_policy = TrustPolicy(
                    ...
                    allowlist=tuple(
                        [
                            s.strip()
                            for s in (
                                getattr(settings, "TRUST_ALLOWLIST", "") or ""
                            ).split(",")
                            if s.strip()
                        ]
                    ),
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(
            self._loop(), name="autonomy_orchestrator_loop"
        )
            ...
            for item in events[-50:]:
                if item.get("role") == "arbiter_refined":
                    ...
                    try:
                        ...
                        answer = payload.get("final_platinum_answer") or payload.get(
                            "answer"
                        )
                    except Exception:
                        # Not JSON; skip
            ...
            for idx, proposal in pending_proposals:
                try:
                    ...
                    if "[action:" in content:
                        try:
                            action_type = content.split("[action:", 1)[1].split("]", 1)[
                                0
                            ]
                        except Exception:
                            action_type = "generic"
