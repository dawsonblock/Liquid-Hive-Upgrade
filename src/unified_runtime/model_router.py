...
class RouterConfig:
    ...
    def from_env(cls) -> "RouterConfig":
        ...
        return cls(
            ...
            max_oracle_tokens_per_day=int(
                os.getenv("MAX_ORACLE_TOKENS_PER_DAY", "250000")
            ),
...
class DSRouter:
    ...
    def _initialize_circuit_breakers(self):
        ...
        self.logger.info(
            "Initialized circuit breakers for providers: %s",
                    ...
                    if circuit_breaker.state == CircuitState.OPEN:
                        try:
                            ...
                            if health_result.get("status") == "healthy":
                                self.logger.info(
                                    f"Provider {provider_name} health recovered, transitioning to HALF_OPEN"
                                )
    ...
    async def _call_provider_with_circuit_breaker(
        self, provider_name: str, request: GenRequest
    ) -> GenResponse:
        ...
        if not circuit_breaker.should_attempt_call():
            raise Exception(
                f"Provider {provider_name} circuit breaker is OPEN (too many failures)"
            )
        ...
        try:
            ...
            self.logger.debug(
                f"Provider {provider_name} call successful, circuit state: {circuit_breaker.state}"
            )
            ...
            self.logger.warning(
                f"Provider {provider_name} call failed, circuit state: {circuit_breaker.state}, error: {e}"
            )
            raise
    ...
    def _initialize_providers(self):
        ...
        try:
            ...
            self.providers["deepseek_thinking"] = DeepSeekThinkingProvider(
                deepseek_config
            )
            ...
            self.logger.info(
                "Initialized DS-Router with providers: %s", list(self.providers.keys())
            )
    ...
    def refresh_config_from_env(self) -> None:
        ...
        try:
            ...
            self.config.hf_token = os.getenv("HF_TOKEN") or os.getenv(
            ...
            try:
                ...
                new_providers["deepseek_thinking"] = DeepSeekThinkingProvider(
                    deepseek_config
                )
            ...
            self._initialize_circuit_breakers()
            self.logger.info(
                "Router config refreshed from env; providers reloaded: %s",
    ...
    async def generate(self, request: GenRequest) -> GenResponse:
        ...
        if self.pre_guard:
            ...
            if pre_guard_result.blocked:
                return self._create_blocked_response(
                    pre_guard_result.reason, start_time
                )
        else:
        ...
        try:
            response = await self._generate_with_routing(
                sanitized_request, routing_decision
            )
        except Exception as e:
        ...
        if (
            routing_decision.provider != "qwen_cpu"
        ):  # Skip confidence check for fallback
            ...
            if (
                confidence < self.config.conf_threshold
                and routing_decision.provider != "deepseek_r1"
            ):
                ...
                try:
                    response = await self._generate_with_routing(
                        sanitized_request, escalation_decision
                    )
                    response.confidence = await self._assess_confidence(
        ...
        if self.post_guard:
            final_response, post_guard_result = await self.post_guard.process(
                response, sanitized_request
            )
            if post_guard_result.blocked:
                return self._create_blocked_response(
                    post_guard_result.reason, start_time
                )
        else:
    ...
    async def _determine_routing(self, request: GenRequest) -> "RoutingDecision":
        ...
        if support_score >= self.config.support_threshold:
            ...
            self.logger.debug(
                f"High RAG support ({support_score:.2f}) - routing to DeepSeek Chat with RAG context"
            )
            ...
            self.logger.debug(
                f"Low RAG support ({support_score:.2f}) - escalating to DeepSeek R1 for novel reasoning"
            )
            return RoutingDecision(
                provider="deepseek_r1",
            ...
            self.logger.debug(
                f"Hard problem with moderate RAG support ({support_score:.2f}) - using DeepSeek Thinking"
            )
            return RoutingDecision(
    ...
    async def _get_rag_support_score(self, prompt: str) -> float:
        ...
        try:
            ...
            self.logger.debug(
                f"RAG support score: {normalized_score:.3f} (raw: {avg_score:.3f})"
            )
    ...
    async def _generate_with_routing(
        self, request: GenRequest, decision: "RoutingDecision"
    ) -> GenResponse:
        """Generate response using the specified routing decision with circuit breaker protection."""
        try:
            response = await self._call_provider_with_circuit_breaker(
    ...
    def _create_budget_exceeded_response(
        self, budget_status: "BudgetStatus", start_time: float
    ) -> GenResponse:
        ...
        for name, provider in self.providers.items():
            try:
                ...
                status[name] = {
                    ...
                    "failure_count": (
                        circuit_breaker.failure_count if circuit_breaker else 0
                    ),
                }
...
class BudgetTracker:
    ...
    async def _check_redis_budget(self) -> BudgetStatus:
        ...
        exceeded = (
            tokens_used >= self.config.max_oracle_tokens_per_day
            or usd_spent >= self.config.max_oracle_usd_per_day
        )
        ...
        self.redis_client.expire(
            daily_key, int((tomorrow - datetime.utcnow()).total_seconds()) + 3600
        )
    ...
    async def _check_fallback_budget(self) -> BudgetStatus:
        ...
        exceeded = (
            self._fallback_tokens >= self.config.max_oracle_tokens_per_day
            or self._fallback_usd >= self.config.max_oracle_usd_per_day
        )

        from datetime import datetime, timedelta

        next_reset = (datetime.utcnow() + timedelta(days=1)).strftime(
            "%Y-%m-%d 00:00:00 UTC"
        )
