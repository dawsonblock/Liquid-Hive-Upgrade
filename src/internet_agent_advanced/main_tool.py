...
async def internet_fetch(urls: List[str], render_js: bool = False) -> FetchResult:
    ...
    for url in urls:
        try:
            ...
            kind = routed["kind"]; data = routed["data"]
            if kind == "web_page":
                score = score_url(data.get("url"))
                item = ScoredItem(url=data.get("url"), score=score, title=data.get("title"), content=data.get("content"), meta={"kind":kind, "status":data.get("status"), "mime":data.get("mime"), "rendered": data.get("rendered"), "elapsed_ms": data.get("elapsed_ms")})
                if data.get("blocked") or data.get("captcha_required"): results.blocked.append(item)
                elif is_trusted(item.url): results.trusted.append(item)
                else: results.unverified.append(item)
            elif kind in ("github_repo","huggingface_model"):
                url_hint = data.get("html_url") if kind=="github_repo" else f"https://huggingface.co/{data.get('repo_id')}"
                score = score_url(url_hint)
                content = data.get("readme") if kind=="github_repo" else str(data.get("model_card"))
                title = data.get("full_name") if kind=="github_repo" else data.get("repo_id")
                item = ScoredItem(url=url_hint, score=score, title=title, content=content, meta={"kind":kind, **{k:v for k,v in data.items() if k not in ('readme','model_card')}})
                if is_trusted(url_hint): results.trusted.append(item)
                else: results.unverified.append(item)
...
async def internet_ingest(url: str, render_js: bool = False) -> dict:
    ...
    return {"url": page.url, "hash": h, "chunks": len(chs), "indexed": idx, "raw_store": loc, "title": meta["title"]}
