...
class PreGuard:
    ...
    def _compile_risk_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "illegal": re.compile(
                r"\b(?:drug|illegal|steal|fraud|hack|exploit|piracy|copyright)\b",
                re.IGNORECASE,
            ),
            ...
            "hate_speech": re.compile(
                r"\b(?:racist|sexist|homophobic|transphobic|hate|discrimination)\b",
                re.IGNORECASE,
            ),
    ...
    async def process(self, request: GenRequest) -> Tuple[GenRequest, PreGuardResult]:
        ...
        injection_detected, injection_reason = self._detect_prompt_injection(
            request.prompt
        )
        ...
        for pii_type, pattern in self.pii_patterns.items():
            ...
            if matches:
                ...
                redacted_text = pattern.sub(
                    f"<REDACTED:{pii_type.upper()}>", redacted_text
                )
    ...
    def _should_block_request(self, risk_flags: List[str]) -> bool:
        ...
        moderate_flags = [
            flag for flag in risk_flags if flag in moderate_risk_categories
        ]
