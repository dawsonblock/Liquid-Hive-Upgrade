...
class PostGuard:
    ...
    def _compile_safety_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "illegal_instructions": re.compile(
                r"\b(?:how to.*(?:steal|hack|fraud|piracy)|instructions.*(?:illegal|criminal))\b",
                re.IGNORECASE,
            ),
            "self_harm_promotion": re.compile(
                r"\b(?:encourage.*(?:suicide|self-harm)|promote.*(?:cutting|ending life))\b",
                re.IGNORECASE,
            ),
            "hate_speech_output": re.compile(
                r"\b(?:all.*(?:race|gender).*are.*(?:inferior|bad|evil)|promote.*(?:discrimination|hatred))\b",
                re.IGNORECASE,
            ),
            "personal_attacks": re.compile(
                r"\b(?:you are.*(?:stupid|idiot|worthless)|insulting.*(?:user|person))\b",
                re.IGNORECASE,
            ),
            "misinformation": re.compile(
                r"\b(?:definitely.*(?:false claim)|certainly.*(?:conspiracy theory)|proven fact.*(?:debunked))\b",
                re.IGNORECASE,
            ),
        }
    ...
    async def process(
        ...
        verified_content, citations_added = await self._verify_and_cite_facts(
            response.content
        )
        ...
        result = PostGuardResult(
            ...
            reason=(
                f"safety_violations: {', '.join(safety_violations)}"
                if should_block
                else ""
            ),
            status=(
                "blocked"
                if should_block
                else ("enhanced" if citations_added else "passed")
            ),
    ...
    def _should_block_response(
        self, safety_violations: List[str], toxicity_score: float
    ) -> bool:
    ...
    def _generate_safe_alternative(self, request: GenRequest) -> str:
        ...
        if len(request.prompt) > 200:
            ...
            word in request.prompt.lower() for word in ["how", "what", "why", "when"]
        ):
