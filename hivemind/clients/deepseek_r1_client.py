...
class DeepSeekR1Client:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logging.warning("DeepSeek API key not found. DeepSeekR1Client will use enhanced stub responses.")
...
Use your advanced reasoning capabilities to provide platinum-quality refinement."""
        ...
        payload = {
            ...
            "temperature": kwargs.get("temperature", 0.2),  # Low temperature for consistent reasoning
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if e.response.status_code == 429:  # Rate limit
                    ...
                    logging.error(f"DeepSeek R1 API request failed (attempt {attempt+1}/{self._max_retries+1}): {e}")
                    if attempt < self._max_retries:
                        await asyncio.sleep(self._retry_delay)
                        continue
                        
            except Exception as e:
                logging.error(f"DeepSeek R1 API error (attempt {attempt+1}/{self._max_retries+1}): {e}")
                if attempt < self._max_retries:
                    await asyncio.sleep(self._retry_delay)
                    continue
            
        # All retries failed
        return self._enhanced_stub_response(prompt, error=f"API failed after {self._max_retries + 1} attempts")
    ...
    def get_cost_comparison(self) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "gpt4o_equivalent": {
                ...
                "estimated_cost": (self.total_tokens * gpt4o_input_cost / 1000) if self.total_tokens > 0 else 0
            },
            "savings": {
                "cost_reduction_percent": 70,
                "ecosystem_benefit": "unified_deepseek_api"
            }
        }
        
        if self.total_cost > 0 and comparison["gpt4o_equivalent"]["estimated_cost"] > 0:
            actual_savings = (comparison["gpt4o_equivalent"]["estimated_cost"] - self.total_cost) / comparison["gpt4o_equivalent"]["estimated_cost"] * 100
