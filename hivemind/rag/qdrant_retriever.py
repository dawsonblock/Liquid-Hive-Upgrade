...
try:
    ...
    from qdrant_client.models import (
        ...
        FieldCondition, Range, MatchValue, UpdateCollection,
        OptimizersConfigDiff, CollectionInfo, SearchRequest,
        Batch, UpdateStatus
    )
    DEPS_AVAILABLE = True
...
class QdrantRetriever:
    ...
    def __init__(self, collection_name: str = "liquid_hive_knowledge", 
                 embed_model_id: str = None, qdrant_url: str = None):
    ...
    def _initialize(self):
        ...
        try:
            ...
            self.is_ready = True
            log.info(f"QdrantRetriever initialized successfully with collection: {self.collection_name}")
        ...
        try:
            ...
            if self.collection_name in collection_names:
                ...
                if actual_dimension != self.embedding_dimension:
                    log.warning(f"Collection dimension mismatch: expected {self.embedding_dimension}, got {actual_dimension}")
                    return
                ...
                log.info(f"Created collection '{self.collection_name}' with dimension {self.embedding_dimension}")
    ...
    async def add_documents(self, file_paths: List[str], 
        ...
        try:
            ...
            log.info(f"Document indexing complete: {len(results['indexed_files'])} files, "
    ...
    def _extract_content(self, path: pathlib.Path) -> str:
        ...
        try:
            ...
            elif path.suffix.lower() in ['.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.yaml', '.yml']:
    ...
    def _detect_content_type(self, path: pathlib.Path, content: str) -> str:
        ...
        if detected == 'plain_text' or detected == 'unknown':
            ...
            if any(keyword in content_lower for keyword in ['def ', 'import ', 'class ', 'return ']):
                detected = 'code'
        ...
        for i in range(0, len(points), self.batch_size):
            ...
            try:
                ...
                if operation_info.status == UpdateStatus.COMPLETED:
                    ...
                    log.warning(f"Batch upload {i//self.batch_size + 1} status: {operation_info.status}")
